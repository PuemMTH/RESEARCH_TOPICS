...
\DeclareUnicodeCharacter{FF1A}{:}  % Add this line
\usepackage{multicol}
\usepackage{multirow} 
% Load hyperref last
...
% \hypersetup{
% pdftitle={SciGraphQA: A Large-Scale Dataset for Scientific Graphs Question-answering with Multi-Modal Language Models},

% pdfsubject={cs.CV, cs.AI}, % Change Subject here
% pdfauthor={Shengzhi Li, Nima Tajbakhsh},
% pdfkeywords={},
% }
...
\begin{abstract}
%While much attention in multi-modal large language models (MLLMs) is focused on natural images, we have dedicated significant effort to understanding graphs such as trend lines, bar plots, and charts. Previous chart-visual-question-answering (chart VQA) datasets tend to use synthetic charts, which limit the diversity of chart images. They also generate questions and answers using restricted pools of question templates and fixed vocabulary.
...
%In this work, we present a synthetic multi-turn question-answer dataset related to academic graphs. Derived from 290,000 Computer Science or Machine Learning ArXiv papers published between 2010 and 2020, we used Palm-2 to generate 295K samples of open-vocabulary multi-turn question-answering dialogues about graphs. A reader of an academic paper often begins with the abstract, glances at key graphs, and reads the caption and paragraph that mentions the figure as context to comprehend the graphs. Inspired by this human reading process, we used rich text contextual data such as the text on the image, title, abstract, and the mentioned paragraph to condition the text-only Palm-2 to generate the dialogues, averaging 2.23 question-answer turns.

%Our dataset, SciGraphQA, is 13 times larger than ChartVQA, the previously largest chart-visual question-answering dataset. It is also the largest open-sourced chart VQA dataset with non-synthetic charts. We asked GPT-4 to evaluate the matching quality of our question-answer turns given the paper's context, obtaining an average rating of 8.7/10 on our 3K test set. We evaluated LLaVa, mPLUGowl, BLIP-2, and openFlamingo's 0-shot performance on our dataset, with LLaVA-13B being the most performant at a 0.08 CIDEr score.

%We leveraged DePlot, an expert chart model, to extract linearized data tables from given images and showed that injecting the extracted data table into the question prompt was useful as a test-time augmentation step, boosting LLaVA's 0-shot CIDEr to 0.15. We also introduced a model fine-tuned on the dataset, SciGraphQA-baseline, reaching a 0.26 CIDEr score. We believed that future work such as segmentation, larger LLM backbone, and prompting techniques could further improve performance. Our code and data are open-sourced at \hyperlink{https://github.com/findalexli/SciGraphQA}{Github} and \hyperlink{https://huggingface.co/datasets/alexshengzhili/SciGraphQA-295K-train}{HuggingFace dataset}.
\end{abstract}
...
% \begin{figure}[!htbp]
% \centering
% \includegraphics[width=13.76cm,height=7.38cm]{./images/image1.png}
% \caption{GPT-4 rating distribution histogram of how accurately the SciGraphQA answer addresses the question given the context of the figure, including the caption, abstract, and paragraph mentioning the figure. Scale from 0 to 10.}
% \label{fig:gpt4_rating_distribution_histogram_gpt4}
% \end{figure}
...
%To assess the importance of the extracted tabular data, {\bf we instructed GPT 3.5 (gpt-3.5-turbo-0631), a text-only model, to attempt to answer the question using only the tabular data extracted by DePlot. 
...
% Alex, can I comment this?
%{\bf We followed the formula introduced by LLaVA, utilizing an existing LLaVA checkpoint.} LLaVA's training consists of two stages: feature alignment and multi-modal instruction fine-tuning. The first stage has the effect of expanding the underlying LLM's vocabulary with visual elements, whereas the second stage trains the LLM to follow instructions to explain graphs given input graphs and questions using a multi-turn question-answering dataset \cite{liu2023}. We utilized the LLaVA-13B checkpoint, released after training on both stages. The causal language modeling loss was computed only at the `Answer', not the `Questions'.
...
\section{Results}
%fed the output of DePlot to GPT 3.5 (text-davinci-003) and Flan-Palm in a similar fashion. DePlot only reported accuracy results using emergent prompting techniques, including self-consistency across 10 predictions and chain-of-thought prompting \cite{liu2022deplot}. Given that our largest MMLM has 13B parameters, which may not exhibit emergent properties such as chain-of-thought the way Flan-PaLM 540B \cite{chung2022scaling} holds and to ensure fairness when compared with other MMLMs, we chose not to reproduce emergent prompting techniques.}


% This figure is redunant 
% \begin{figure}[!htbp]
% \centering
% \includegraphics[width=13.76cm,height=6.67cm]{./images/image3.png}
% \caption{Evaluation of Large Multi-modal models performance on CIDERr, BLEU-4, and ROUGE on the SciGraphQA 3K test set}
% \end{figure}
...
% Noteworthy, OpenFlamingo lagged behind other 7B MLMMs. We attribute this inferior performance to how Flamingo is optimized for in-context learning with Retrieval-based in-context example selection (RICE) \cite{yang2022empirical}. Flamingo retrieves similar images given the query image based on image encodings, and top-N (image, text) examples are added to the prompt \cite{alayrac2022flamingo}. Given the scope of the paper, we were not able to reproduce this recipe, instead, we randomly selected 3, 6, and 9 examples from the training set. Preliminary comparisons between randomly-selected few-shot and 0-shot OpenFlamingo did not show a performance gain.
...
%We started with a pre-trained checkpoint of LLaVA-13B, which is built on the LLAMA-2-13B-chat backbone. This checkpoint was trained using a dataset of 558K pre-training instances, coupled with an 88K LLaVA instruction-tuning dataset. This model serves as our baseline, from which further training was carried out using LoRa adapters and an unfrozen linear projector from the image encoder.
...
%We introduced the SciGraphQA dataset, a resource distinguished by its unique characteristics. It was derived from actual academic publications and focused on multi-turn question-answering dialogues, which set it apart from the preceding similar datasets. Furthermore, containing 300K samples, SciGraphQA marks a notable 13-fold increase over the size of ChartQA, underscored its worth as a crucial resource for evaluating and training large multi-modal models. Despite the synthetic generation of dialogues by Palm-2, the system was fed comprehensive textual context including the paper's title, abstract, and the paragraph containing the graph. The quality of responses relative to the questions was further verified by an impressive 8.8/10 GPT-4 average rating on a test set. The development of multimodal large language models could greatly benefit from this dataset as a multimodal instruction tuning resource, thereby enhancing their capacity to answer questions about scientific graphs. %This dataset presented a valuable resource for the research community, who could leverage it to supplement their training data or fine-tune their models to achieve better performance using the SciGraphQA dataset.
...
%From a modeling perspective, we  employed a fixed CLIP/ViT as the vision encoder for our 0-shot assessments and fine-tuning results, with mPLUG-owl being the sole exception. Given that CLIP had been extensively pre-trained on enormous set of web-sourced images, enabling the encoder to adapt during training could have potentially improved the integration of visual elements into the LLM input domain, specifically for graphs. There could have been a necessity for spatial awareness in identifying specific legends, trends, or labels to comprehend compositional questions like ``where do the blue and green lines intersect?". Importantly, as CLIP was trained to align visual embeddings with  captions, its spatial awareness abilities may be limited. We hypothesize that \textbf{segmentation-based approaches}, such as employing the Segment-Anything Model \cite{kirillov2023segment} to extract masks and project them into the LLM token embedding space, could further improve the visual-question-answering capability of the MLLMs.
...
%In this study, we confined our evaluations to conventional NLP metrics which only took into account pairs of ground truth reference answers and model-generated predictions. Arguably, GPT-4 could be harnessed as an evaluation tool due to its capability in evaluating the model's predicted answers based on the entire text context in conjunction with the posed questions. In our experimentation with GPT-4, however, we encountered three notable challenges. First, inconsistent results were observed when asking for a rating directly and when soliciting ``step by step" thinking in the Chain-of-thought prompting style. Second, limited resources constrained us to run only 100 visual question-answering prompts for GPT-4 evaluations (for context, the authors of LLaVa used only 30 samples for their quantitative evaluation \cite{liu2023}), hindering conclusive comparisons between the models. 
%Third,  the non-deterministic property of GPT-4 evaluation could render subsequent work on SciGraphQA incomparable with our published results. Therefore, we reported only the CIDEr, ROUGE, and BLEU-4 scores, which are freely accessible, open, and reproducible. We would like to stress that further exploration of GPT-4 as an evaluation tool is a worthwhile endeavor and could potentially be integrated as a reward model for reinforcement learning.
% STOPPING POINT

%@alex, this should go to discussion or conclusion?
%In this series of 0-shot evaluations, we demonstrated that prompt augmentation such as inserting an extracted chart table is an important design choice to augment large, foundational models applied to out-of-distribution data, boosting the best 0-shot LLaVA model from 0.08 CIDEr to 0.15 CIDEr. Our fine-tuned model, SciGraphQA-baseline, demonstrated  higher performance than models not trained on SciGraphQA (0.27 vs 0.15 CIDEr), serving as a reference baseline for future work.
...
%We postulate that a Flamingo-style architecture \cite{alayrac2022flamingo} \cite{yang2022empirical}, capable of multiple image inputs, could efficiently utilize the Retrieval-based In-context Example (RICE) method.  Such an approach offers a potentially cost-effective route for further adapting MLLM to specialized domains like scientific graphs, without resorting to training the LLM backbone.
% This study could be considered as a work towards adapting MLLMs to handle out-of-distribution data, in our case scientific graph question-answering. At a high level, the practice of fine-tuning large, foundation models can be expensive and fraught with sensitivity to hyperparameters and the quality of input data, both of which could lead to catastrophic forgetting. Our empirical tests indicated that when the prompt was enhanced with a data table derived from an expert model, performance improved considerably. As we look forward, we postulated that the adoption of a Flamingo-style architecture, which can handle multiple image inputs, might be key to harnessing the power of Retrieval-based In-context Example (RICE) \cite{yang2022empirical}\cite{alayrac2022flamingo}. However, the image encoder may require fine-tuning to accommodate out-of-distribution domains. We highlighted the potential of using external models and in-context retrieval augmentation to enrich prompts, which might offer a more cost-efficient approach for scaling MLLM to specialized domains like Scientific Graphs, as opposed to training the LLM backbone.themehyperparameter choice and quality of input data which could both result in catastrophic forgetting. need and in-context retrieval augmentationcost-effective such as Scientific Graphs. 
...
%Our analyses revealed the challenges that scientific graphs pose for state-of-the-art MLLMs, underscoring the value of SciGraphQA as a benchmark to advance multimodal reasoning. We demonstrated the benefits of prompt-augmentation using expert models like DePlot and the potential for accuracy gains by fine-tuning on this dataset.


% This work introduced SciGraphQA, a large-scale, 295K multi-turn question-answering multi-modal dataset containing 59.1 million tokens, designed specifically for multi-turn dialogues pertaining to scientific graphs. We constructed question-answer turns using the paper's textual context and validated the fidelity of our question-answer pairs with an average rating of 8.6/10 using GPT-4. Our evaluation of current MLLMs (multi-modal large language models) on the dataset found that employing a larger LLM backbone and utilizing prompt-augmentation with the assistance of an expert graph data-extraction model boosted their performance, without the need for fine-tuning the model. Additionally, we introduced SciGraphQA-baseline, a model fine-tuned on the dataset, which achieved a CIDEr score of 0.27. This dataset can be used as a benchmark for MLLM. Looking forward, we also believe the community could utilize this dataset as a open-source multi-modal instruction-tuning resource.
