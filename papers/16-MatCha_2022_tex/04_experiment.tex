...
%So, we should regard Statista as the more reliable and less noisy evaluation set.

%\juliancomment{Probably a table with some info about dataset sizes and also an example input output for each one would be useful here.} TODO for the Jan. 20th version
...
%Pix2Struct-large  & no & 83.8 & 33.4 & 58.6 & \\
...
% CRCT \citep{levy2022classification}
%\vspace{-0.5em}
\label{tab:main}
\end{table*}
...
Overall, \model{} is clearly the best-performing model with SOTA or competitive performance on every setup and all tasks. %Overall, \model{} outperforms all models on every QA task, even models with access to gold table.
All baselines without access to gold tables lag behind significantly -- \model{} outperforms the strongest baseline without gold table access Pix2Struct by $\approx10\%$ if we average the performance scores across all datasets.
...
% The finding suggests that the multimodal community should pay more attention on visual language data which is ubiquitous but beyond the natural image domain.
...
% - only Pix2Struct screenshot parsing continue pretrain & 82.5 & 31.6 & 57.0 \\
\bottomrule
\end{tabular}
}
%\vspace{-0.5em}
...
%\paragraph{The impact of individual datasets/tasks.}
...
%To understand more concretely what capabilities \model{} pretraining has brought to the model and what is still lacking, we present several case studies to highlight the strengths of \model{} and an area for improvement. We also list predictions by baselines Pix2Struct and PaLI (res. 558) for comparison. The examples are all picked from the ChartQA dev/test sets.
