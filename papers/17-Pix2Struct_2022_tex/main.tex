...
% \author{Kenton Lee\thanks{equal contributions and corresponding authors},~
% Mandar Joshi\rlap{,}$^*$
% Iulia Turc\thanks{currently at succinctly.ai; work done at Google},~
% Hexiang Hu,
% Fangyu Liu\thanks{currently at University of Cambridge; work done during Google internship},
% Julian Eisenschlos,\\
% \textbf{Urvashi Khandelwal, Peter Shaw, Ming-Wei Chang, Kristina Toutanova}\\
% Google Research\\
% \texttt{\{kentonl, mandarj\}@google.com}\\
% }
\end{icmlauthorlist}
...
\centering
\input{figures/tasks}
\caption{Examples of visually-situated language understanding tasks, including diagram QA (AI2D), app captioning (Screen2Words), and document QA (DocVQA). We also include an example of our proposed pretraining task (screenshot parsing) on the left.~\ourmodel~encodes the pixels from the input image (above) and decodes the output text (below).}
\vspace{-10pt}
\label{fig:tasks}
\end{figure*}
...
% Table~\ref{tab:ablations} shows that all components of the pretraining scheme are crucial for good downstream task performance across all benchmarks.
...
\begin{figure}[!t]
\input{figures/aspect_ratio}
\caption{Our variable-resolution inputs prevent aspect-ratio distortion while minimizing padding.
}
...
% \begin{figure*}[ht]
% \begin{center}
% \includegraphics[width=\textwidth, keepaspectratio]{figures/input_rep.pdf}
% \end{center}
% \caption{Comparison of our variable resolution inputs and the typical fixed resolution input. We illustrate the preprocessing for a target sequence length of 36 patches for both inputs.}
% \label{fig:input_rep}
% \end{figure*}
