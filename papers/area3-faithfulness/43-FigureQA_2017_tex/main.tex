...
\glsdisablehyper % removes hyper links for acronyms and glossary!!

% PLEASE ENTER ACCURACIES INTO THESE MACROS
% models validated on val2:
\def\bestcnnvalone{58.21} % cnn on val 1
\def\bestcnnvaltwo{56.16} % cnn on val 2
\def\bestrnvalone{76.39} % rn on val 1
\def\bestrnvaltwo{72.54} % rn on val 2
\def\bestvggvalone{-} % vgg on val 1
\def\bestvggvaltwo{52.31} % vgg on val 2
\def\besttextvalone{49.85} % text on val 1
\def\besttextvaltwo{50.01} % text on val 2

\def\bestrntestone{76.52} %  rn on test one
\def\bestrntesttwo{72.40} % rn on test two
\def\bestcnntesttwo{56.00} %  cnn on test two
\def\bestvggtesttwo{52.47} %  vgg on test two
\def\besttexttesttwo{50.01} %  text on test two
% human subset
\def\besthumantesttwohuman{91.21} %  humans
\def\bestcnntesttwohuman{56.04} %  cnn on human set
\def\bestrntesttwohuman{72.18} % rn on human set
% ---
% rn early stopped on val1
\def\bestrnonevalone{67.95} % rn(val1) on val 1
\def\bestrnonevaltwo{60.60} % rn(val1) on val 2
\def\bestrnonetestone{67.74} % rn(val1) on test 1
\def\bestrnonetesttwo{60.35} % rn(val1) on test 2
...
%   \texttt{samira.ebrahimi@microsoft.com} \\
%   \And
%   Adam Atkinson \\
%   Microsoft Research Montréal \\
%   \texttt{adatkins@microsoft.com} \\
%   \And
%   Vincent Michalski \\
%   Université de Montréal, MILA\thanks{Part of this work was completed while the author interned at Microsoft Research Montréal.} \\
%   \texttt{vincent.michalski@umontreal.ca} \\
%   \And
%   Ákos Kádár \\
%   Tilburg University$^\ast$ \\
%   \texttt{kadar.akos@gmail.com} \\
%   \And
%   Adam Trischler \\
%   Microsoft Research Montréal \\
%   \texttt{adam.trischler@microsoft.com} \\
%   \And
%   Yoshua Bengio \\
...
\begin{table}[h]
    ...
    \begin{center}
        \begin{tabular}{lcc}
            ...
            \midrule
            Text only & \besttextvaltwo & \besttexttesttwo \\ % 1.5M
            CNN+LSTM & \bestcnnvaltwo & \bestcnntesttwo \\ % 1.5M
            CNN+LSTM on VGG-16 features & \bestvggvaltwo & \bestvggtesttwo \\ % 1.5M
            \gls{rn} & \bestrnvaltwo & \bestrntesttwo \\ % 1.5M
            \bottomrule
        \end{tabular}
    \end{center}
\end{table}
...
\begin{table}[h]
    ...
    \begin{center}
        \begin{tabular}{lc}
            ...
            \gls{rn} & \bestrntesttwohuman \\ % 1.5M
            Human & \besthumantesttwohuman \\
            \bottomrule
        \end{tabular}
    \end{center}
\end{table}
...
\begin{table}[h]
    ...
    \begin{center}
        \begin{tabular}{lccc}
            ...
            \midrule
            Vertical Bar & 59.63 & 77.13 %& 65.74 
            & 95.90 \\
            Horizontal Bar & 57.69 & 77.02 % & 63.12 
            & 96.03 \\
            Line & 54.46 & 66.69 % & 57.59 
            & 90.55 \\
            Dot Line & 54.19 & 69.22 % & 57.83 
            & 87.20 \\
            Pie & 55.32 & 73.26 % & 65.92 
            & 88.26 \\
            \bottomrule
        \end{tabular}
    \end{center}
...
\begin{table}[h]
    ...
    \begin{center}
        \begin{tabular}{clccc}
            ...
            \midrule
            1 & Is $X$ the minimum?         & 56.63 & 76.78 % & 65.67   
                                                        & 97.06 \\
            2 & Is $X$ the maximum?         & 58.54 & 83.47 % 67.55   
                                                        & 97.18 \\
            3 & Is $X$ the low median?      & 53.66 & 66.69 % 58.35   
                                                        & 86.39 \\
            4 & Is $X$ the high median?     & 53.53 & 66.50 % 57.85   
                                                        & 86.91 \\
            5 & Is $X$ less than $Y$?       & 61.36 & 80.49 % 69.70   
                                                        & 96.15 \\
            6 & Is $X$ greater than $Y$?    & 61.23 & 81.00 % 70.58   
                                                        & 96.15 \\
            7 & Does $X$ have the minimum area under the curve? & 56.60 & 69.57 % 59.50   
                                                                        & 94.22 \\
            8 & Does $X$ have the maximum area under the curve? & 55.69 & 78.45 % 58.45   
                                                                        & 95.36 \\
            9 & Is $X$ the smoothest?       & 55.49 & 58.57 % 58.94   
                                                                        & 78.02 \\
            10 & Is $X$ the roughest?       & 54.52 & 56.28 % 59.18   
                                                                        & 79.52 \\
            11 & Does $X$ have the lowest value?     & 55.08 & 69.65 % 60.08
                                                                        & 90.33 \\
            12 & Does $X$ have the highest value?    & 58.90 & 76.23 % 58.78   
                                                                        & 93.11 \\
            13 & Is $X$ less than $Y$?\tablefootnote{\label{note:humanstrictly} In the sense of \emph{strictly greater/less} than. This clarification is provided to judges for the human baseline.}       & 50.62 & 67.75 % 55.05 
                                                        & 90.12 \\
            14 & Is $X$ greater than $Y$?\footref{note:humanstrictly}      & 51.00 & 67.12 % 54.86 
                                                        & 89.88 \\
            15 & Does $X$ intersect $Y$?     & 49.88 & 68.75 % 53.78 
                                                        & 89.62 \\
            \bottomrule
        \end{tabular}
    \end{center}
...
\label{sec:samples}
% Prevent the splitting of the word 'annotations'
...
\begin{table}[h]
    ...
    \begin{center}
        \begin{tabular}{lcc}
            ...
            \midrule
            \gls{rn} (val1) & \bestrnonetestone & \bestrnonetesttwo \\ % 1.5M
            \gls{rn} (val2) & \bestrntestone & \bestrntesttwo \\ % 1.5M
            \bottomrule
        \end{tabular}
    \end{center}
\end{table}
