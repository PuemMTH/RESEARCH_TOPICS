...
\textbf{แนวทาง:} เพิ่ม 2 objectives ใน finetuning บน Pix2Struct: (1) Math reasoning (arithmetic, counting); (2) Chart derendering (chart$\rightarrow$table)\\
...
\textbf{แนวทาง:} แยกเป็น 2 module: DePlot (chart$ightarrow$table, fine-tuned) + LLM (table$ightarrow$answer, one-shot)\\
...
\begin{longtable}{@{}C{0.4cm}L{2.8cm}C{0.8cm}L{2.6cm}L{3.0cm}L{3.2cm}C{0.6cm}C{0.6cm}C{0.6cm}@{}}
  ...
  \rowcolor{rowA}3 & DePlot & 2023 & Plot$ightarrow$Table$ightarrow$LLM & +29.4\% ChartQA one-shot & QA-only, ไม่ใช่ captioning & — & — & — \\
...
\textbf{$ightarrow$ โอกาสวิจัย: ระบบที่รับ scientific figure image + retrieved paper context $ightarrow$ สร้าง caption ที่ match author style}
\end{gapbox}
...
\textbf{$ightarrow$ โอกาสวิจัย: Cross-paper author profile ด้วย RAG ที่ retrieve ผลงานที่ผ่านมาของ author คนเดียวกัน}
\end{gapbox}
...
\textbf{$ightarrow$ โอกาสวิจัย: Fine-tune chart VLM ด้วย scientific figure data หรือใช้ domain adaptation}
\end{gapbox}
...
\textbf{$ightarrow$ โอกาสวิจัย: Multi-dimensional evaluation: (1) scientific accuracy, (2) style fidelity, (3) informativeness}
\end{gapbox}
...
\textbf{$ightarrow$ โอกาสวิจัย: Multimodal RAG ที่รับ figure image เป็น query และ retrieve paper sections/related figures}
\end{gapbox}
