...
\begin{center}
  ...
  {\large การสร้างคำอธิบายรูปภาพและกราฟในเอกสารวิชาการ}\\[2pt]
  {\large และ Retrieval Augmented Generation (RAG)}\\[4pt]
  {\color{muted}\small Figure \& Chart Captioning · RAG: Naïve to Agentic}
\end{center}
...
รวมถึงเทคนิค Retrieval Augmented Generation (RAG) ที่เสริม LLM
ด้วยความรู้จากแหล่งข้อมูลภายนอก
โดยสามารถจัดกลุ่มได้เป็น 4 แนวทางหลัก:
\textbf{(1)} การสร้างชุดข้อมูลขนาดใหญ่,
\textbf{(2)} การแปลง plot เป็นข้อความเพื่อ reasoning,
\textbf{(3)} การใช้ LLM แบบหลายตัวร่วมกัน, และ
\textbf{(4)} RAG แบบ Naive จนถึง Agentic
...
%% ─────────────────────────────────────────────────────────────────
\section*{Paper 6 — RAG: Naïve to Agentic (2026)}

\noindent{\color{coral}\textbf{ชื่อ:}} RAG: Naïve to Agentic\\
\noindent{\color{muted}\small Sirapakit Lim (Tony) · IPU, NECTEC · Knowledge Sharing \#3 · เมษายน 2026}

\vspace{8pt}
\setlength{\parindent}{2em}
เอกสารนี้เป็น slide deck สำหรับ knowledge sharing ที่ครอบคลุมเทคนิค
\textbf{Retrieval Augmented Generation (RAG)} ตั้งแต่รูปแบบพื้นฐาน
จนถึงระดับ Agentic โดยมีเนื้อหา 4 ส่วนหลัก:
\textbf{(1)} ข้อจำกัดของ LLM ที่ RAG แก้ไข (knowledge cutoff, hallucination, private data),
\textbf{(2)} Basic RAG — retriever + knowledge base + augmented prompt,
\textbf{(3)} RAG Improvement — chunking strategies, query parsing (HyDE, NER, rewriting), reranking (bi-encoders, cross-encoders, ColBERT),
และ \textbf{(4)} Agentic RAG ที่ LLM ทำหน้าที่ตัดสินใจเลือกเครื่องมือเอง
มีการกล่าวถึงการรองรับภาษาไทยด้วยการใช้ \texttt{pythainlp: attacut}
สำหรับ tokenization ในขั้นตอน chunking

\vspace{6pt}
\noindent\textbf{จุดเด่น:}
\begin{itemize}[nosep, leftmargin=2em]
  \item ครอบคลุม RAG ทั้งสาย: Basic $\to$ Improvement $\to$ Agentic
  \item เปรียบเทียบ Keyword Search (TF-IDF, BM25) กับ Semantic Search (Vector)
  \item กล่าวถึง Thai NLP — ใช้ \texttt{pythainlp: attacut} tokenize ก่อน chunk
  \item แนะนำ RAGAS library สำหรับ evaluation และ LangGraph สำหรับ Agentic RAG
  \item เหมาะเป็น foundation สำหรับ context-aware scientific figure retrieval
\end{itemize}
